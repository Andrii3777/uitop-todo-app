...
describe('Todos API (e2e)', () => {
  ...
  describe('PATCH /todos/:id', () => {
    it('toggles completed status (R3)', async () => {
      const create = await request(app.getHttpServer())
        .post('/todos')
        .send({ text: 'Toggle me', categoryId: 2 })
        .expect(201);
  ...
  describe('5-task limit per category', () => {
    ...
    it('allows new task when 5 are completed but 0 active (KTD3)', async () => {
      const listRes = await request(app.getHttpServer())
        .get(`/todos?category=${CAT_ID}`)
        .expect(200);

      const activeTodos = (
        listRes.body as { id: number; completed: boolean }[]
      ...
      await request(app.getHttpServer())
        .post('/todos')
        .send({ text: 'New task after completing all', categoryId: CAT_ID })
        .expect(201);
    });
  });
});
